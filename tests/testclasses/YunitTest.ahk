  ...
  Class Util {
    ...
    Class Types {
      ...
      isArray_should_check_if_var_is_not_an_array() {
        Yunit.expect(Yunit.Util.IsArray([])).toBe(true)
      ...
      getType_should_return_the_correct_variable_type() {
        ...
        Yunit.expect(Yunit.Util.GetType(Yunit.Util())).toBe("Yunit.Util")
        ...
        vt_empty := ComValue(0, &empty := {}, 1)
        Yunit.expect(Yunit.Util.GetType(vt_empty)).toBe("ComValue")
      }

      isFunction_should_determine_whether_an_object_is_callable() {
        Yunit.expect(Yunit.Util.IsFunction(Substr)).toBe(true)
        Yunit.expect(Yunit.Util.IsFunction(Substr.bind())).toBe(true)
